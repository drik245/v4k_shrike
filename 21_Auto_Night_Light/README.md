...
An intelligent night light system that uses an LDR (Light Dependent Resistor) to monitor ambient brightness. When the room gets dark, the LED breathes gracefully. When the room is bright, the LED glows steadily.
...
- **Consistent Analog Reads:** By using `analogReadResolution(8)`, both the ESP32-S3 and RP2040 output LDR values in the 0-255 range, making the threshold logic perfectly portable across both microcontrollers.
...
> 3. Cover the LDR with your hand (simulating darkness) and observe the values dropping.
> 4. Shine a light on it (simulating daylight) and observe the values rising.
